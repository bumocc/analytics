defmodule PlausibleWeb.Live.AuthContext do
  ...
  def on_mount(:default, _params, session, socket) do
    # Plugin session auth may provide a user or subscription redirect
    plugin_user = plugin_session_user(session)
    ...
    socket =
      ...
      |> assign_new(:current_user, fn
        %{current_user_session: nil} when plugin_user != nil ->
          plugin_user
    ...
    # Subscription check for center-server users
    case subscription_redirect(socket.assigns[:current_user]) do
      nil ->
        socket =
          socket
      ...
      url ->
        {:halt, Phoenix.LiveView.redirect(socket, external: url)}
    end
  end

  defp plugin_session_user(session) do
    try do
      case Plausible.Auth.Plugin.authenticate_from_session(session) do
        {:ok, user} -> user
        _ -> nil
      end
    rescue
  ...
  defp subscription_redirect(user) do
    try do
      case Plausible.Auth.CenterServerPlugin.require_subscription!(user) do
        {:error, url} -> url
        :ok -> nil
      end
    rescue
